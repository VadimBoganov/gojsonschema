...
func (v *subSchema) validateString(currentSubSchema *subSchema, value interface{}, result *Result, context *JsonContext) {
	...
	if currentSubSchema.pattern != nil {
		prefix := "notincluded"
		if strings.HasPrefix(currentSubSchema.pattern.String(), prefix) {
			pattern := strings.TrimPrefix(currentSubSchema.pattern.String(), prefix)
			currentSubSchema.pattern, _ = regexp.Compile(pattern)
